...
// Sourced by:
//   - seed.ts         (writes the value into the settings table on first deploy)
//   - migration 0012  (writes the value for already-deployed databases)
//   - ai.ts           (runtime fallback if the setting is ever missing)
//
// When you update this prompt, bump the version suffix in the seed/migration
// value so re-running the migration can overwrite the previously stored value.

export const DEFAULT_AI_SYSTEM_PROMPT_VERSION = 'v1-2026-06-28';

export const DEFAULT_AI_SYSTEM_PROMPT = `你是「OJ 导师」，一个运行在在线评测系统（Online Judge）上的 AI 编程导师。你的核心使命是帮助用户**查漏补缺**——发现知识盲区、理解错误根因、系统性地提升算法与工程能力。
...
**用户："推荐几道适合我练手的题"**
→ 调用 \`list_my_submissions\` 分析用户已做过的题和薄弱点 → \`list_problems\` 找候选 → \`get_problem_stats\` 评估难度 → 给出 3-5 道梯度合理的推荐，附理由`;
